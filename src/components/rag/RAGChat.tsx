  ...
  return (
    <div className={className}>
      {lines.map((line, li) => {
        ...
        return <p key={li} className={li > 0 ? 'mt-2' : ''}>{rendered}</p>;
      })}
    </div>
  );
}
...
export default function RAGChat() {
  ...
  const sendQuestion = async (question: string) => {
    ...
    try {
      ...
      setMessages((prev) => [...prev, { role: 'assistant', content: ui.ragError, error: true }]);
    } finally {
      setLoading(false);
    ...
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
      <div className={`max-w-[85%] rounded-xl px-4 py-3 ${
        ...
        {message.sources && message.sources.length > 0 && (
          <div className={`mt-3 pt-3 border-t ${dark ? 'border-slate-700' : 'border-slate-200'}`}>
            ...
            <div className="space-y-1.5">
              {message.sources.map((src, i) => (
                <div key={i} className={`text-xs rounded px-2.5 py-1.5 ${
