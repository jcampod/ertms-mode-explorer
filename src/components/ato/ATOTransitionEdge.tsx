...
const ATOTransitionEdge = ({
  ...
  const getStrokeColor = () => {
    ...
    if (state === 'highlighted' || state === 'hovered') return '#60a5fa';
    return idleColor;
  };
  ...
  const getMarkerEnd = () => {
    if (state === 'highlighted' || state === 'hovered') {
      return 'url(#ato-arrowhead-active)';
    }
    return theme === 'dark' ? 'url(#ato-arrowhead)' : 'url(#ato-arrowhead-light)';
  };

  const dashArray = transition.triggerType === 'both' ? '3 3' : (transition.triggerType === 'driver' ? '6 4' : undefined);
...
export default ATOTransitionEdge;
